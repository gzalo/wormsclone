...
tuple<const SDL_Surface*, GLuint> loadImage(const string &filename){
    ...
    const SDL_Surface* loadedImage = SDL_CreateRGBSurfaceWithFormatFrom((void*)data, width, height,
...
int drawRect(int img, int x, int y, int w, int h){
    ...
    glBegin(GL_QUADS);
    glTexCoord2d(0.0, 0.0); glVertex3f( x0, y0, 0.0f);
    glTexCoord2d(1.0, 0.0); glVertex3f( x1, y0, 0.0f);
    glTexCoord2d(1.0, 1.0); glVertex3f( x1, y1, 0.0f);
    glTexCoord2d(0.0, 1.0); glVertex3f( x0, y1, 0.0f);
    glEnd();
